...
async def create_comment(post_id: int, comment: CommentCreate, db: AsyncSession = Depends(get_db),
    ...
    if not current_user:
        raise HTTPException(
            ...
            detail="Authentication required. Please log in to write a comment."
        )
    return await posts_comments.create_comment(db, post_id, comment, current_user)


@router.put("/comments/{comment_id}", response_model=CommentResponse)
async def update_comment(comment_id: int, comment_update: CommentUpdate, db: AsyncSession = Depends(get_db),
                         current_user: User = Depends(get_current_user)):
    updated_comment = await posts_comments.update_comment(db, comment_id, comment_update, current_user)
...
async def delete_comment(comment_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    result = await posts_comments.delete_comment(db, comment_id, current_user)
    if not result:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Comment not found")
